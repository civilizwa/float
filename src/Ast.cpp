...
void UnaryExpr::genCode()
{
    ...
    else if (op == NOT)
    {
        if (expr->getType()->isInt())
        {
            new CmpInstruction(CmpInstruction::E, dst, expr->getOperand(), new Operand(new ConstantSymbolEntry(TypeSystem::intType, 0)), builder->getInsertBB());
        }
        else if (expr->getType()->isFloat())
        {
...
void CallExpr::genCode()
{
    ...
    new CallInstruction(dst, symbolEntry, rParams, builder->getInsertBB());
}
...
void Id::genCode()
{
    ...
    if (se->getType()->isArray())
    {
        ...
        if (this->isPointer)
        {
            ...
            new GepInstruction(dst, addr, offs, builder->getInsertBB());
            return;
        }
...
void ImplictCastExpr::genCode()
{
    ...
    if (op == BTI)
    {
        ...
        new ZextInstruction(dst, expr->getOperand(), true, builder->getInsertBB());
    }
    // int转bool
    else if (op == ITB)
    {
        new CmpInstruction(CmpInstruction::NE, dst, expr->getOperand(), new Operand(new ConstantSymbolEntry(TypeSystem::intType, 0)), builder->getInsertBB());
    }
    else if (op == FTI)
    {
        new F2IInstruction(dst, expr->getOperand(), builder->getInsertBB());
    }
    else if (op == ITF)
    {
        new I2FInstruction(dst, expr->getOperand(), builder->getInsertBB());
    }
    else if (op == FTB)
    {
...
void DeclStmt::genCode()
{
    ...
    {
        BasicBlock *entry = builder->getInsertBB()->getParent()->getEntry();
        Instruction *alloca;
        Operand *addr;
        ...
        if (se->isParam())
        {
            builder->getInsertBB()->getParent()->addParam(se->getArgAddr());
            new StoreInstruction(addr, se->getArgAddr(), builder->getInsertBB());
        }
        if (se->getType()->isArray() && exprArray) // 如果数组有初始化
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                {
                    exprArray[i]->genCode();
                    new StoreInstruction(ele_addr, exprArray[i]->getOperand(), builder->getInsertBB());
                }
                else
                {
                    new StoreInstruction(ele_addr, zeroReg, builder->getInsertBB());
                }
                if (i != size - 1)
                {
                    Operand *next_addr = new Operand(new TemporarySymbolEntry(new PointerType(new ArrayType({}, eleType)), SymbolTable::getLabel()));
                    new GepInstruction(next_addr, ele_addr, {step}, builder->getInsertBB(), true);
                    ele_addr = next_addr;
                }
                // new BinaryInstruction(BinaryInstruction::ADD, ele_addr, ele_addr, step, builder->getInsertBB());
            }
            // for (int i = 0; i < size; i++) {
            //     if (exprArray[i]) {
...
void IfStmt::genCode()
{
    ...
    then_bb = new BasicBlock(builder->getInsertBB()->getParent());
    end_bb = new BasicBlock(builder->getInsertBB()->getParent());
...
void IfElseStmt::genCode()
{
    ...
    then_bb = new BasicBlock(builder->getInsertBB()->getParent());
    else_bb = new BasicBlock(builder->getInsertBB()->getParent());
    end_bb = new BasicBlock(builder->getInsertBB()->getParent());
...
void WhileStmt::genCode()
{
    ...
    cond_bb = new BasicBlock(builder->getInsertBB()->getParent());
    stmt_bb = new BasicBlock(builder->getInsertBB()->getParent());
    end_bb = new BasicBlock(builder->getInsertBB()->getParent());
...
void BreakStmt::genCode()
{
    if (whileStmt)
    {
        new UncondBrInstruction(((WhileStmt *)whileStmt)->get_end_bb(), builder->getInsertBB());
...
void ContinueStmt::genCode()
{
    if (whileStmt)
    {
        new UncondBrInstruction(((WhileStmt *)whileStmt)->get_cond_bb(), builder->getInsertBB());
        builder->setInsertBB(new BasicBlock(builder->getInsertBB()->getParent()));
    }
}
...
void ReturnStmt::genCode()
{
    ...
    new RetInstruction(retValue ? retValue->getOperand() : nullptr, builder->getInsertBB());
}
...
void FunctionDef::genCode()
{
    ...
    for (auto b = func->begin(); b != func->end(); b++)
    {
        ...
        { // 如果这个块没有后继，而且没有ret
            if (((FunctionType *)builder->getInsertBB()->getParent()->getSymPtr()->getType())->getRetType()->isInt())
            {
                new RetInstruction(new Operand(new ConstantSymbolEntry(TypeSystem::intType, 0)), (*b));
            }
            if (((FunctionType *)builder->getInsertBB()->getParent()->getSymPtr()->getType())->getRetType()->isFloat())
            {
                new RetInstruction(new Operand(new ConstantSymbolEntry(TypeSystem::floatType, 0)), (*b));
            }
            else if (((FunctionType *)builder->getInsertBB()->getParent()->getSymPtr()->getType())->getRetType() == TypeSystem::voidType)
            {
